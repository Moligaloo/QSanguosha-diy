...
void SkillBox::insertSuit(int index){
    ...
    QString suit_path = QString("cardsuit/%1.png").arg(suit_name);
...
QLayout *CardEditor::createGeneralLayout(){
    ...
    QStringList kingdom_names;
    kingdom_names << "wei" << "shu" << "wu" << "qun";

    foreach(QString kingdom, kingdom_names){
        QIcon icon(QString("diy/%1-icon.png").arg(kingdom));
...
QWidget *CardEditor::createSkillBox(){
    ...
    for(i=0; i<4; i++){
        ...
        QIcon suit_icon(QString("cardsuit/%1.png").arg(suit_name));
